...
class PHDDataset(BaseDataset):
    ...
    def _load_images_paths(self,):
        if self.isTrain:
            ...
            path = self.opt.content
        if os.path.isdir(path):
            self.paths_all = list(map(str, Path(path).glob('*')))
        else:
        ...
        if self.isTrain:
            ...
            self.path_img = self.paths_all
            if os.path.isdir(self.opt.content):
                self.path_mask = list(map(str, Path(self.opt.mask).glob('*')))
            else:
                self.path_mask = [self.opt.mask]
            ## style
            if os.path.isdir(self.opt.style):
                self.path_style = list(map(str, Path(self.opt.style).glob('*')))
            else:
                self.path_style = [self.opt.style]

            self.path_img.sort()
            self.path_mask.sort()
            self.path_style.sort()
